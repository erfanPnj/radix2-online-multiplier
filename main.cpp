...
string format_w_assimilated(const int S[], const int C[], int SZ, int frac_bits)
{
    int sum_arr[200] = {0};
    int carry = 0;
    for (int i = SZ - 1; i >= 0; --i)
    ...
    string res = to_string(shifted[0]) + to_string(shifted[1]) + ".";
    for (int i = 0; i < frac_bits; ++i)
    {
        res += to_string(shifted[i + 2]);
    }
    return res;
}
...
// Selector
template <size_t N>
void select_val(int input_digit, const CAReg<N> &reg, int A[], int SZ, int &c_out, int &lsb_idx_out)
{
    for (int i = 0; i < SZ; ++i)
        A[i] = 0;
...
public:
    void run(const vector<int> &x_in, const vector<int> &y_in)
    {
        ...
        for (int j = -3; j < (int)N; ++j)
        {
            ...
            string s_w = format_w_assimilated(next_W_S, next_W_C, SZ, frac_bits);
        ...
        for (size_t i = 0; i < x_in.size(); ++i)
            expected_x += x_in[i] * pow(2.0, -(int)(i + 1));
        for (size_t i = 0; i < y_in.size(); ++i)
            expected_y += y_in[i] * pow(2.0, -(int)(i + 1));

        double exact_product = expected_x * expected_y;
        double truncation_error = abs(exact_product - final_result);

        cout << "\n================================ FINAL RESULTS ===============================\n";
        cout << "Hardware P_out Sequence : 0.";
        ...
        cout << "Computed SD Result (Dec): " << final_result << "\n";
        cout << "Expected x_in * y_in    : " << expected_x << " * " << expected_y << " = " << exact_product << "\n";
        cout << "Truncation Error        : " << truncation_error << "\n";
        cout << "Target Error Bound      : " << pow(2.0, -(int)N) << " (2^" << -(int)N << ")" << "\n";
        cout << "==============================================================================\n";
    }
};
